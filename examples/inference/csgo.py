...
def get_model():
    image_encoder_path: str = "IP-Adapter/sdxl_models/image_encoder"
    controlnet_path: str = "TTPLANET_Controlnet"
    csgo_ckpt: str = "CSGO/csgo_4_32.bin"

    prefix_path = os.path.join(PARENT_PATH, "data/weights")

    config = CSGOConfig(
        image_encoder_path=os.path.join(prefix_path, image_encoder_path),
        controlnet_path=os.path.join(prefix_path, controlnet_path),
        csgo_ckpt=os.path.join(prefix_path, csgo_ckpt),
    )
    model = CSGOModel(config, device="cuda")
    return model
...
def main(
    ...
    if output_path is None:
        output_path = os.path.join(PARENT_PATH, "output/inference/csgo")
