...
def get_depth_map(
    image: Image,
    feature_processor: DPTImageProcessor,
...
def concat_zero_control(control_reisduel: T) -> T:
    ...
    return torch.cat(
        (zerso_reisduel, control_reisduel[:b], zerso_reisduel, control_reisduel[b::])
    )
...
def controlnet_call(
    ...
    controlnet = (
        pipeline.controlnet._orig_mod
        if is_compiled_module(pipeline.controlnet)
    ...
    if isinstance(controlnet, ControlNetModel):
        ...
        image = torch.stack(
            [image[0]] * num_images_per_prompt + [image[1]] * num_images_per_prompt
        )
    else:
        assert False
    # 5. Prepare timesteps
    ...
    prompt_embeds = torch.stack(
        [prompt_embeds[0]] + [prompt_embeds[1]] * num_images_per_prompt
    )
    negative_prompt_embeds = torch.stack(
        [negative_prompt_embeds[0]]
        + [negative_prompt_embeds[1]] * num_images_per_prompt
    )
    negative_pooled_prompt_embeds = torch.stack(
        [negative_pooled_prompt_embeds[0]]
        + [negative_pooled_prompt_embeds[1]] * num_images_per_prompt
    )
    add_text_embeds = torch.stack(
        [add_text_embeds[0]] + [add_text_embeds[1]] * num_images_per_prompt
    )
    ...
    controlnet_prompt_embeds = torch.cat(
        (prompt_embeds[1:batch_size], prompt_embeds[1:batch_size])
    )
    controlnet_added_cond_kwargs = {
        key: torch.cat((item[1:batch_size,], item[1:batch_size]))
        for key, item in added_cond_kwargs.items()
    }
    with pipeline.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            control_model_input = torch.cat(
                (
                    latent_model_input[1:batch_size,],
                    latent_model_input[batch_size + 1 :],
                )
            )
            ...
            if cond_scale > 0:
                ...
                down_block_res_samples = [
                    concat_zero_control(down_block_res_sample)
...
def panorama_call(
    ...
    # prepare batch grid
    views = pipeline.get_views(
        height, width, circular_padding=circular_padding, stride=stride
    )
    views_batch = [
        views[i : i + view_batch_size] for i in range(0, len(views), view_batch_size)
    ]
    views_scheduler_status = [copy.deepcopy(pipeline.scheduler.__dict__)] * len(
        views_batch
    )
    ...
    negative_prompt_embeds = torch.cat(
        [negative_prompt_embeds[:1], *[negative_prompt_embeds[1:]] * view_batch_size]
    )
    prompt_embeds = torch.cat(
        [prompt_embeds[:1], *[prompt_embeds[1:]] * view_batch_size]
    )
    ...
    with pipeline.progress_bar(total=num_inference_steps) as progress_bar:
        for i, t in enumerate(timesteps):
            ...
            for j, batch_view in enumerate(views_batch):
                ...
                # expand the latents if we are doing classifier free guidance
                latent_reference_plus_view = torch.cat(
                    (reference_latent, latents_for_view)
                )
                latent_model_input = latent_reference_plus_view.repeat(2, 1, 1, 1)
                prompt_embeds_input = torch.cat(
                    [
                        negative_prompt_embeds[: 1 + vb_size],
    ...
    reference_image = pipeline.vae.decode(
        reference_latent / pipeline.vae.config.scaling_factor, return_dict=False
    )[0]
    ...
    image = pipeline.image_processor.postprocess(
        image, output_type="pil", do_denormalize=[True]
    )
    reference_image = pipeline.image_processor.postprocess(
        reference_image, output_type="pil", do_denormalize=[True]
    )
    pipeline.maybe_free_model_hooks()
