...
def _encode_text_sdxl(
    model: StableDiffusionXLPipeline, prompt: str
) -> tuple[dict[str, T], T]:
...
def _encode_image(model: StableDiffusionXLPipeline, image: np.ndarray) -> T:
    ...
    latent = (
        model.vae.encode(image.to(model.vae.device))["latent_dist"].mean
...
def _next_step(
    model: StableDiffusionXLPipeline, model_output: T, timestep: int, sample: T
) -> T:
    ...
    alpha_prod_t = (
        model.scheduler.alphas_cumprod[int(timestep)]
        if timestep >= 0
...
def make_inversion_callback(zts, offset: int = 0) -> [T, InversionCallback]:
    ...
    def callback_on_step_end(
        pipeline: StableDiffusionXLPipeline, i: int, t: T, callback_kwargs: dict[str, T]
    ) -> dict[str, T]:
