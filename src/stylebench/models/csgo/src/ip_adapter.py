...
class IPAdapter_CS:
    def __init__(
        ...
        if content_image_encoder_path is not None:
            self.content_image_encoder = AutoModel.from_pretrained(
                content_image_encoder_path
            ).to(self.device, dtype=torch.float16)
            self.content_image_processor = AutoImageProcessor.from_pretrained(
                content_image_encoder_path
            )
        else:
            self.content_image_encoder = CLIPVisionModelWithProjection.from_pretrained(
        ...
        self.clip_image_processor = CLIPImageProcessor()
        # image proj model
        self.content_image_proj_model = self.init_proj(
    ...
    def init_proj(self, num_tokens, content_or_style_="content", model_resampler=False):
        ...
        if (
            content_or_style_ == "content"
            and self.content_image_encoder_path is not None
        ):
            image_proj_model = ImageProjModel(
                cross_attention_dim=self.pipe.unet.config.cross_attention_dim,
            ...
            if self.controlnet_adapter is False:
                if isinstance(self.pipe.controlnet, MultiControlNetModel):
                    for controlnet in self.pipe.controlnet.nets:
                        controlnet.set_attn_processor(
                            CNAttnProcessor(
                                num_tokens=self.num_content_tokens
                                + self.num_style_tokens
                            )
                        )
                else:
                ...
                for name in self.pipe.controlnet.attn_processors.keys():
                    ...
                    cross_attention_dim = (
                        None
                        if name.endswith("attn1.processor")
                        else self.pipe.controlnet.config.cross_attention_dim
                    )
                    if name.startswith("mid_block"):
                        ...
                        block_id = int(name[len("up_blocks.")])
                        hidden_size = list(
                            reversed(self.pipe.controlnet.config.block_out_channels)
                        )[block_id]
                    elif name.startswith("down_blocks"):
                        block_id = int(name[len("down_blocks.")])
                        hidden_size = self.pipe.controlnet.config.block_out_channels[
                            block_id
                        ]
                    if cross_attention_dim is None:
                        # layername_id += 1
                        ...
                        for block_name in controlnet_content_target_blocks:
                            if block_name in name:
                                if selected is False:
                                    ...
                                    controlnet_attn_procs[name].set_content_ipa(
                                        content_scale=1.0
                                    )
    ...
    def load_ip_adapter(self):
            ...
            with safe_open(self.ip_ckpt, framework="pt", device="cpu") as f:
                for key in f.keys():
                    if key.startswith("content_image_proj."):
                        state_dict["content_image_proj"][
                            key.replace("content_image_proj.", "")
                        ] = f.get_tensor(key)
                    elif key.startswith("style_image_proj."):
                        state_dict["style_image_proj"][
                            key.replace("style_image_proj.", "")
                        ] = f.get_tensor(key)
                    elif key.startswith("ip_adapter."):
                        state_dict["ip_adapter"][key.replace("ip_adapter.", "")] = (
                            f.get_tensor(key)
        ...
        if "conv_in_unet_sd" in state_dict.keys():
            self.pipe.unet.conv_in.load_state_dict(
                state_dict["conv_in_unet_sd"], strict=True
            )
        ip_layers = torch.nn.ModuleList(self.pipe.unet.attn_processors.values())
        ip_layers.load_state_dict(state_dict["ip_adapter"], strict=False)
        ...
        if self.controlnet_adapter is True:
            ...
            self.pipe.controlnet.load_state_dict(
                state_dict["controlnet_adapter_modules"], strict=False
            )
    ...
    def get_image_embeds(
        ...
        if content_or_style_ == "content":
            if pil_image is not None:
                ...
                if self.content_image_proj_model is not None:
                    clip_image = self.content_image_processor(
                        images=pil_image, return_tensors="pt"
                    ).pixel_values
            ...
            uncond_image_prompt_embeds = self.content_image_proj_model(
                torch.zeros_like(clip_image_embeds)
            )
            return image_prompt_embeds, uncond_image_prompt_embeds
        if content_or_style_ == "style":
            if pil_image is not None:
                if self.use_CSD is not None:
                    clip_image = (
                        self.style_preprocess(pil_image)
                        .unsqueeze(0)
            ...
            uncond_image_prompt_embeds = self.style_image_proj_model(
                torch.zeros_like(clip_image_embeds)
            )
            return image_prompt_embeds, uncond_image_prompt_embeds
    ...
    def generate(
        ...
        **kwargs,
        ...
        if pil_content_image is not None:
            num_prompts = (
                1
                if isinstance(pil_content_image, Image.Image)
                else len(pil_content_image)
            )
        else:
        ...
        content_image_prompt_embeds, uncond_content_image_prompt_embeds = (
            self.get_image_embeds(
                pil_image=pil_content_image, clip_image_embeds=clip_content_image_embeds
            )
        )
        style_image_prompt_embeds, uncond_style_image_prompt_embeds = (
            self.get_image_embeds(
        ...
        content_image_prompt_embeds = content_image_prompt_embeds.repeat(
            1, num_samples, 1
        )
        content_image_prompt_embeds = content_image_prompt_embeds.view(
            bs_embed * num_samples, seq_len, -1
        )
        uncond_content_image_prompt_embeds = uncond_content_image_prompt_embeds.repeat(
            1, num_samples, 1
        )
        uncond_content_image_prompt_embeds = uncond_content_image_prompt_embeds.view(
            bs_embed * num_samples, seq_len, -1
        )
        ...
        style_image_prompt_embeds = style_image_prompt_embeds.view(
            bs_embed * num_samples, seq_style_len, -1
        )
        uncond_style_image_prompt_embeds = uncond_style_image_prompt_embeds.repeat(
            1, num_samples, 1
        )
        uncond_style_image_prompt_embeds = uncond_style_image_prompt_embeds.view(
            bs_embed * num_samples, seq_style_len, -1
        )
        ...
        with torch.inference_mode():
            ...
            negative_prompt_embeds = torch.cat(
                [
                    ...
                    uncond_content_image_prompt_embeds,
...
class IPAdapterXL_CS(IPAdapter_CS):
    ...
    def generate(
        ...
        **kwargs,
        ...
        num_prompts = (
            1 if isinstance(pil_content_image, Image.Image) else len(pil_content_image)
        )
        ...
        content_image_prompt_embeds, uncond_content_image_prompt_embeds = (
            self.get_image_embeds(
                pil_content_image, content_image_embeds, content_or_style_="content"
            )
        )

        style_image_prompt_embeds, uncond_style_image_prompt_embeds = (
            self.get_image_embeds(
                pil_style_image, style_image_embeds, content_or_style_="style"
            )
        )
        ...
        with torch.inference_mode():
            ...
            prompt_embeds = torch.cat(
                [prompt_embeds, content_image_prompt_embeds, style_image_prompt_embeds],
                dim=1,
            )
            negative_prompt_embeds = torch.cat(
                [
...
class CSGO(IPAdapterXL_CS):
    ...
    @torch.inference_mode()
    def get_image_embeds(
        self, pil_image=None, clip_image_embeds=None, content_or_style_=""
    ):
        if isinstance(pil_image, Image.Image):
            pil_image = [pil_image]
        if content_or_style_ == "style":
            ...
            if self.content_image_encoder_path is not None:
                ...
                outputs = self.content_image_encoder(
                    clip_image.to(self.device, dtype=torch.float16),
                    output_hidden_states=True,
                )
                ...
                if self.content_model_resampler:
                    ...
                    # clip_image_embeds = clip_image_embeds.to(self.device, dtype=torch.float16)
                    image_prompt_embeds = self.content_image_proj_model(
                        clip_image_embeds
                    )
                    # uncond_clip_image_embeds = self.image_encoder(
                    #             torch.zeros_like(clip_image), output_hidden_states=True
