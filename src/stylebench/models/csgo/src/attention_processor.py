...
class IP_CS_AttnProcessor2_0(torch.nn.Module):
    ...
    def set_content_ipa(self, content_scale=1.0):
        ...
        self.to_k_ip_content = nn.Linear(
            self.cross_attention_dim or self.hidden_size, self.hidden_size, bias=False
        )
        self.to_v_ip_content = nn.Linear(
            self.cross_attention_dim or self.hidden_size, self.hidden_size, bias=False
        )
        self.content_scale = content_scale
        self.content = True
    ...
    def __call__(
        ...
        if encoder_hidden_states is None:
            ...
            encoder_hidden_states, ip_content_hidden_states, ip_style_hidden_states = (
                encoder_hidden_states[:, :end_pos, :],
                encoder_hidden_states[
                    :, end_pos : end_pos + self.num_content_tokens, :
                ],
        ...
        if not self.skip and self.content is True:
            ...
            ip_content_key = ip_content_key.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            ip_content_value = ip_content_value.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            ...
            ip_content_hidden_states = ip_content_hidden_states.transpose(1, 2).reshape(
                batch_size, -1, attn.heads * head_dim
            )
            ip_content_hidden_states = ip_content_hidden_states.to(query.dtype)

            hidden_states = (
                hidden_states + self.content_scale * ip_content_hidden_states
            )
        ...
        if not self.skip and self.style is True:
            ...
            ip_style_key = ip_style_key.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            ip_style_value = ip_style_value.view(
                batch_size, -1, attn.heads, head_dim
            ).transpose(1, 2)
            ...
            ip_style_hidden_states = ip_style_hidden_states.transpose(1, 2).reshape(
                batch_size, -1, attn.heads * head_dim
            )
